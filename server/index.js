...
const pool = mysql.createPool({
    ...
    database: 'heroku_da53ac18454eade',
...
app.get('/test', (req, res) => {
    res.json({ state: '연결되었음' });
})

app.get('/', (req, res) => {
    res.send('movie server is up and running');
})
...
const getPasswordHashByUsername = async (username) => {
    const QUERY = "SELECT password FROM simplemovie_logindb where username = ?";
...
const usernameDoesExist = async (username) => {
    const QUERY = 'SELECT username FROM simplemovie_logindb where username = ?';
    const [rows] = await promisePool.query(QUERY, username);
...
const storeUserInfo = async (username, password) => {
    const QUERY = 'INSERT INTO simplemovie_logindb(username, password, date) VALUES (?,?,current_timestamp())';
